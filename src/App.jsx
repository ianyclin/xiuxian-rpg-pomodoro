...
const ARTIFACT_POOL = [
  ...
  { id: 'a02', rarity: 'COMMON', name: '青銅戈', desc: '凡兵銳氣。戰力加成 +2%', val: { atk: 0.02 } },
  ...
  { id: 'a62', rarity: 'DIVINE', name: '涅槃真血', desc: '真靈不死。氣血+50%，氣運+1.0，免死+10%/級', val: { hp: 0.50, luck_floor: 1.00, revive: 0.10 } },
...
export default function App() {
  ...
  const [player, setPlayer] = useState(() => {
    try {
      const saved = localStorage.getItem('xianxia_master_v68_final');
  ...
  useEffect(() => { 
    localStorage.setItem('xianxia_master_v68_final', JSON.stringify(player)); 
  ...
  const handleHeal = () => {
    if (player.coins >= healCost && player.vitality < maxVitality) {
      ...
      addLog(`[煉丹] 吞服回春丹，氣血瞬間恢復 50%。`);
    }
  };

  const handleGiveUp = () => {
    setIsActive(false); setTargetEndTime(null);
    if (Math.random() < evadeRate) { addLog(`💨 【羅煙閃避】遁入虛空，成功避開天道反噬！`); }
    else {
      ...
      if (nextHp <= 0) {
        if (Math.random() < reviveRate) { nextHp = maxVitality; addLog(`✨ 【涅槃重生】命不該絕，重生歸來！`); }
  ...
  const handleComplete = () => {
    ...
    if (mode === 'focus') {
      ...
      const passiveQi = Math.floor(50 * Math.pow(1.18, player.realmIndex + 1) * getMultiplier('qi') * (focusDuration / 1500));
      const passiveCoin = Math.floor(50 * Math.pow(1.15, player.realmIndex + 1) * getMultiplier('stone') * currentLuck * (focusDuration / 1500));
      ...
      if (newHp === 0) {
        ...
        } else { killLog = `⚔️ 【擊殺】斬殺妖獸！奪得修為 ${killQi}，靈石 ${killCoin}。` + killLog; }
      ...
      addLog(killLog !== '' ? `${dmgLog} ${killLog}` : `${dmgLog} 獲修為 ${passiveQi}。`);
    ...
    <div className={`min-h-screen text-slate-300 font-mono p-4 flex flex-col items-center overflow-x-hidden relative transition-colors duration-300 
      ...
      <div className="w-full max-w-4xl mb-6 transition-focus z-10 font-bold px-2 md:px-0">
        ...
        <div className={`bg-slate-900/50 backdrop-blur-3xl p-4 md:p-6 rounded-xl border ${themeBorderClass} relative shadow-2xl`}>
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/5 pb-6 mb-6">
            ...
            <div className="flex flex-row flex-wrap md:flex-nowrap justify-center md:justify-end items-end gap-x-6 md:gap-x-8 gap-y-3 w-full md:w-auto font-mono text-xs">
               ...
               <div className="flex flex-col items-center md:items-end min-w-[70px] text-rose-500 font-bold"><span className="text-[7px] uppercase font-black"><Sword size={8}/> 連擊</span><span className={comboMultiplier > 2.0 ? 'animate-pulse' : ''}>x{comboMultiplier.toFixed(2)}</span></div>
        ...
        <div className="mt-6 flex flex-col gap-4 font-bold">
           <div className="flex overflow-x-auto py-4 px-4 gap-6 bg-black/30 rounded-xl border border-white/5 flex-shrink-0 custom-scrollbar">{REALMS.map((r, i) => (<div key={i} className={`flex flex-col items-center min-w-[80px] transition-all relative ${i===player.realmIndex?'scale-110 opacity-100':'opacity-30'}`}><div className={`w-8 h-8 rounded border flex items-center justify-center font-black text-[10px] rotate-45 transition-all ${i===player.realmIndex?'bg-white text-black rotate-0 shadow-2xl':'border-white/10 text-white'}`}><span>{i}</span></div><span className="text-[8px] mt-6 whitespace-nowrap uppercase">{r.name}</span></div>))}</div>
      ...
      <div className={`w-full max-w-4xl mt-4 transition-all duration-500 z-10 font-bold ${isActive ? 'opacity-0 pointer-events-none translate-y-10' : 'opacity-100 translate-y-0'}`}>
          ...
          <div className="p-4 md:p-8 overflow-y-auto flex-1 custom-scrollbar">
            {activeTab === 'skills' && (
              <div className="space-y-12 animate-pop-in">
                ...
                <div><h3 className="text-white/50 text-[10px] font-black uppercase border-b border-white/10 pb-4 mb-8">機緣秘籍 (參悟)</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{SECRET_BOOKS.map(book => { const lvl = player.secretBooks?.[book.id] || 0; return (<div key={book.id} className={`p-5 rounded-xl border transition-all flex flex-col justify-between h-56 ${lvl > 0 ? 'bg-emerald-950/30 border-emerald-500/40 shadow-lg' : 'bg-black/50 border-white/5 opacity-40'}`}><div className="flex items-center gap-4"><div className={`p-4 rounded-lg ${lvl > 0 ? 'bg-emerald-500 text-black shadow-lg' : 'bg-slate-800'}`}><BookOpen size={20}/></div><div><h4 className="font-black text-sm tracking-widest text-white">{book.name} {lvl > 0 && <span className="text-[9px] opacity-50 ml-2 font-mono">Lv.{lvl}</span>}</h4><p className="text-[10px] opacity-60 mt-1 text-white">{lvl > 0 ? book.desc : '尚未參透此法。'}</p></div></div>{lvl > 0 && lvl < 5 && <button onClick={() => { const cost = Math.floor(10000 * Math.pow(2, lvl) * forgeDiscount); if(player.coins >= cost) setPlayer(p => ({...p, coins: p.coins - cost, secretBooks: {...p.secretBooks, [book.id]: lvl+1}})) }} disabled={player.coins < Math.floor(10000 * Math.pow(2, lvl) * forgeDiscount)} className="mt-4 w-full py-2 bg-white/10 hover:bg-emerald-500 text-white hover:text-black rounded text-[9px] font-black border border-white/10 transition-all disabled:opacity-30">參悟升級</button>}</div>);})}</div></div>
              </div>
            )}
            {activeTab === 'forge' && (
              <div className="space-y-12 animate-pop-in pb-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-bold">
                  <div className="p-8 rounded-xl bg-emerald-950/30 border border-emerald-500/30 h-64 flex flex-col justify-between shadow-lg"><div><h3 className="text-emerald-400 font-black text-xl flex items-center gap-2"><Pill size={20}/> 回春丹</h3><p className="text-white/60 text-[10px] mt-2 italic">恢復 50% 氣血。</p></div><button onClick={handleHeal} disabled={player.coins < healCost || player.vitality >= maxVitality} className="w-full py-5 bg-emerald-900/60 hover:bg-emerald-600 text-emerald-100 rounded font-black text-xs border border-emerald-500/50">煉丹 ({healCost} 靈石)</button></div>
                ...
                <div className="space-y-6">
                   <h3 className="text-white/40 text-[10px] font-black uppercase border-b border-white/10 pb-4 tracking-widest">陣法守禦</h3>
        ...
        <footer className="pt-16 pb-24 text-center text-[9px] font-light text-white/40 tracking-[0.8em] uppercase flex flex-col items-center gap-4">
          <p>《凡人修仙傳》原著設定歸作者 忘語 所有 | V68 完美收官版</p>
          <button onClick={()=>{if(window.confirm('確定執行？數據無法恢復。')) { localStorage.clear(); window.location.reload(); }}} className="opacity-50 hover:opacity-100 transition-opacity border border-white/20 px-4 py-1.5 rounded-full text-[8px] tracking-widest hover:bg-rose-900/50">散功重修</button>
        </footer>
      </div>
    </div>
  );
