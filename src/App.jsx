...
// ✨ 專屬道侶高亮渲染器 (原著個性化色彩版)
  const renderCompanionLog = (text) => {
    if (!text) return null;
    const companionNames = COMPANIONS.map(c => c.name).join('|');
    const regex = new RegExp(`(【(?:${companionNames})】)`, 'g');
    const parts = text.split(regex);

    // 依照原著道侶個性與功法設定的專屬色盤
    const colorMap = {
      "【陳巧倩】": "text-yellow-300 drop-shadow-[0_0_8px_rgba(253,224,71,0.8)]",   // 溫婉、黃楓谷
      "【董萱兒】": "text-rose-400 drop-shadow-[0_0_8px_rgba(251,113,133,0.8)]",     // 嬌蠻、紅拂女之徒
      "【南宮婉】": "text-purple-300 drop-shadow-[0_0_8px_rgba(216,180,254,0.8)]",   // 正宮、輪迴訣 (高貴紫)
      "【元瑤】":   "text-teal-300 drop-shadow-[0_0_8px_rgba(94,234,212,0.8)]",     // 陰陽輪迴訣、鬼修 (幽冥青)
      "【紫靈仙子】": "text-fuchsia-400 drop-shadow-[0_0_8px_rgba(232,121,249,0.8)]", // 絕世容顏 (魅惑紫紅)
      "【銀月】":   "text-sky-200 drop-shadow-[0_0_8px_rgba(186,230,253,0.8)]",     // 銀月狼族 (皎潔銀藍)
      "【冰鳳】":   "text-blue-300 drop-shadow-[0_0_8px_rgba(147,197,253,0.8)]",     // 冰鳳凰 (清冷冰藍)
      "【寶花始祖】": "text-pink-500 drop-shadow-[0_0_8px_rgba(236,72,153,0.8)]"      // 魔族始祖 (玄妙粉)
    };
    ...
    return (
      <span>
        {parts.map((part, i) => {
          if (part.match(regex)) {
            // 若匹配到姓名，給予專屬顏色，否則給予預設粉色
            const colorClass = colorMap[part] || "text-pink-400 drop-shadow-[0_0_8px_rgba(244,114,182,0.8)]";
            return <span key={i} className={`${colorClass} font-black px-0.5`}>{part}</span>;
          }
...
{/* --- 狀態主面板 --- */}
        <div className={`bg-slate-900/50 backdrop-blur-3xl p-5 md:p-8 rounded-xl border ${activeColorClass.border} relative z-[120] shadow-2xl transition-all duration-500`}>
...
{/* 懸浮/點按展開之演算公式面板 (極簡定案版) */}
   <div className="absolute top-full right-0 mt-2 w-64 bg-black/95 border border-rose-500/50 rounded-xl p-4 shadow-[0_0_30px_rgba(244,63,94,0.3)] opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus:opacity-100 group-focus:visible transition-all duration-300 z-[100] pointer-events-none backdrop-blur-md text-left">
      <div className="text-[10px] text-rose-400/60 mb-1 border-b border-rose-500/20 pb-1 font-black tracking-widest uppercase flex justify-between">
         <span>天道演算．連擊增傷（上限 800％）</span>
      </div>
      
      <div className="text-[10px] text-rose-300/40 mb-4 font-mono tracking-tighter leading-relaxed">
         = 層數 x 基礎增傷（5%）x 效率加成
      </div>
      
      <div className="text-xs font-mono text-rose-200 flex flex-col gap-3">
         <div className="flex justify-between items-center">
            <span className="text-white/60 font-bold">當前層數</span>
            <span>{player.streakCount || 0} 層</span>
         </div>
         
         <div className="flex justify-between items-center">
            <span className="text-white/60 font-bold">效率加成</span>
            <span>x{streakEff.toFixed(2)}</span>
         </div>
         
         <div className="flex justify-between items-center">
            <span className="text-white/60 font-bold">目前連擊上限</span>
            <span>+{(streakCap * 100).toFixed(0)}%</span>
         </div>
...
{activeTab === 'log' && (
              <div className="space-y-4 md:space-y-6 animate-pop-in pb-10">
                {(player.logs || []).map((e, i) => (
                  <div key={i} className={`p-5 md:p-6 rounded-xl border leading-relaxed transition-all whitespace-pre-wrap text-xs md:text-sm ${
                    i === 0 
                      ? 'bg-white/20 text-white shadow-xl animate-pulse border-white/20' 
                      : 'bg-black/60 border-white/10 text-white/70'
                  }`}>
                    {renderCompanionLog(e)}
                  </div>
                ))}
              </div>
            )}
