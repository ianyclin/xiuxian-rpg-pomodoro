...
const ARTIFACT_POOL = [
  ...
  { id: 'a40', rarity: '黑風旗', desc: '空間隱匿。閃避率+15%，減傷+15%/級', val: { evade: 0.15, def: 0.15 } },
  ...
  { id: 'a52', rarity: 'MYTHIC', name: '虛天大鼎', desc: '鼎鎮靈界。減傷+80%，氣運保底+20%/級', val: { def: 0.80, luck_floor: 0.20, forge_discount: 0.40 } },
...
export default function App() {
  ...
  const [player, setPlayer] = useState(() => {
    try {
      const saved = localStorage.getItem('xianxia_master_v62_final');
  ...
  useEffect(() => { 
    localStorage.setItem('xianxia_master_v62_final', JSON.stringify(player)); 
  ...
  const getMultiplier = (type) => {
    ...
    (player.artifacts || []).forEach(id => { const art = ARTIFACT_POOL.find(a => a.id === id); const lvl = 1 + (player.artifactLvls?.[id] || 0) * 0.5; if (art) processItem(art, lvl); });
  ...
  const handleHeal = () => {
    if (player.coins >= healCost && player.vitality < maxVitality) {
      ...
      addLog(`[煉丹] 吞服丹藥，瞬間恢復 50% 氣血。`);
    }
  };

  const handleGiveUp = () => {
    setIsActive(false); setTargetEndTime(null);
    if (Math.random() < evadeRate) { addLog(`💨 【羅煙閃避】成功避開反噬！`); }
    else {
  ...
  const handleComplete = () => {
    ...
    if (mode === 'focus') {
      ...
      if (newHp === 0) {
        ...
        } else { killLog = `⚔️ 【擊殺】斬殺妖獸！獲額外修為 ${killQi}。` + killLog; }
  ...
  const handleGacha = () => {
    ...
    } else { setPlayer(p => ({ ...p, coins: p.coins - gachaCost, qi: p.qi + 100 })); addLog(`[萬寶樓] 未獲異寶，領取補償修為。`); }
  };
  ...
  const handleUpgradeArtifact = (artId, rarity) => {
    ...
    if (player.coins >= cost && currentLvl < 5) { 
      ...
      addLog(`[血煉] 祭煉法寶，提升至 Lv.${currentLvl + 1}。`);
    }
  };
  ...
  return (
    <div className={`min-h-screen text-slate-300 font-mono p-4 flex flex-col items-center overflow-x-hidden relative transition-colors duration-300 
      ...
      <div className="pointer-events-none fixed inset-0 z-[500] flex items-center justify-center overflow-hidden">
        ...
        {(player.vitality / maxVitality < 0.3) && <div className="absolute inset-0 border-[20px] border-red-600/10 animate-pulse shadow-[inset_0_0_80px_rgba(220,38,38,0.4)]"></div>}
      </div>
        ...
        <div className={`bg-slate-900/50 backdrop-blur-3xl p-4 md:p-6 rounded-xl border ${themeBorderClass} relative shadow-2xl`}>
          <div className="flex flex-col md:flex-row justify-between items-center gap-6 border-b border-white/5 pb-6 mb-6">
            ...
            <div className="flex flex-row flex-wrap md:flex-nowrap justify-center md:justify-end items-end gap-x-6 md:gap-x-8 gap-y-3 w-full md:w-auto">
               <div className="flex flex-col items-center md:items-end min-w-[65px] font-bold"><span className="text-[7px] text-yellow-500 uppercase flex items-center gap-1"><Coins size={8}/> 靈石</span><span className="text-sm text-yellow-500 font-mono font-bold">{Math.floor(player.coins)}</span></div>
               <div className="flex flex-col items-center md:items-end min-w-[50px] font-bold"><span className="text-[7px] text-cyan-400 uppercase flex items-center gap-1"><Zap size={8}/> SP</span><span className="text-sm text-cyan-400 font-mono font-bold">{availableSP}</span></div>
               <div className="flex flex-col items-center md:items-end min-w-[70px] font-bold"><span className="text-[7px] text-rose-500 uppercase flex items-center gap-1"><Sword size={8}/> 連擊</span><span className={`text-sm text-rose-500 font-mono font-bold transition-all duration-500 ${comboMultiplier > 2.0 ? 'text-rose-300 scale-110 animate-pulse drop-shadow-[0_0_10px_rgba(244,63,94,0.8)]' : ''}`}>x{comboMultiplier.toFixed(2)}</span></div>
               <div className="flex flex-col items-center md:items-end min-w-[70px] font-bold"><span className="text-[7px] text-emerald-400 uppercase flex items-center gap-1 font-bold"><Clover size={8}/> 氣運</span><span className={`text-sm text-emerald-400 font-mono font-bold transition-all duration-500 ${getMultiplier('luck_floor') > 1.5 ? 'text-yellow-400 scale-110 animate-bounce' : ''}`}>x{getMultiplier('luck_floor').toFixed(2)}</span></div>
            </div>
          </div>
      ...
      <div className={`w-full max-w-4xl mt-4 transition-all duration-500 z-10 font-bold ${isActive ? 'opacity-0 pointer-events-none translate-y-10' : 'opacity-100 translate-y-0'}`}>
              ...
              <div className="space-y-12 animate-pop-in pb-10">
                ...
                <div className="space-y-6">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white/5 p-6 rounded-xl border border-white/10 h-48 flex flex-col justify-between shadow-inner"><div className="flex justify-between text-sm text-white font-bold">聚靈大陣 <span className="opacity-50 font-mono">Lv.{player.arrays?.qi||0}</span></div><p className="text-[10px] opacity-60 italic">靈氣獲獲 +5%/級</p><button onClick={() => { if(player.coins >= arrayQiCost) setPlayer(p => ({ ...p, coins: p.coins - arrayQiCost, arrays: {...p.arrays, qi: (p.arrays?.qi||0)+1} })) }} disabled={player.coins < arrayQiCost} className="w-full py-3 bg-white/10 hover:bg-white text-white rounded text-[9px] font-black border border-white/10 transition-all">升級 ({arrayQiCost} 靈石)</button></div>
        ...
        <footer className="pt-16 pb-24 text-center text-[9px] font-light text-white/40 tracking-[0.8em] uppercase flex flex-col items-center gap-4">
          <p>《凡人修仙傳》設定歸原作者 忘語 所有 | V62 終極定製版</p>
          <button onClick={()=>{if(window.confirm('散功後所有成果遺失。確定？')) { localStorage.clear(); window.location.reload(); }}} className="opacity-50 hover:opacity-100 transition-opacity border border-white/20 px-4 py-1.5 rounded-full text-[8px] tracking-widest hover:bg-rose-900/50 hover:text-rose-200">散功重修</button>
        </footer>
      </div>
    </div>
  );
