...
const CHANGELOG_DATA = [
  { version: "v6.6.0", title: "天道圓滿・萬象歸一", desc: "確立最終數值基石，抹平 UI 與邏輯的最後縫隙。", changes: [ "優化【尋寶次數顯示】：Footer 與按鈕現在會統一加總「每日免費」與「保底次數」。", "強化【音效容錯】：修正了特定行動裝置下音效播放可能導致的微小延遲。", "封裝【因果法則】：再次校驗 Karma 點數發放與天賦疊加邏輯，確保多周目穩定性。" ] }
];
...
export default function App() {
  ...
  const getMultiplier = (type) => {
    ...
    if (player.activeCompanion) {
        ...
        if (comp && comp.buffType === type) {
            const k = player.companionKills?.[comp.id] || 0;
  ...
  const handleReincarnation = () => {
    ...
    let msg = isNascentSoul ? `【兵解輪迴】保留成就並獲取 ${earnedKarma} Karma？` : "【天道無情】神魂不足以帶走因果，確定轉生？";
    ...
    if (mode === 'focus') {
      ...
      if (newHp === 0) {
        ...
        if (Math.random() < evadeRate) actionLogs.push(`💨 【反撲】閃避了【${atkName}】！`);
            ...
            if (nextVitality <= 0) {
                if (nextShields > 0) { nextShields -= 1; nextVitality = Math.floor(maxVitality * 0.1); actionLogs.push(`🛡️ 【反護】法寶鎖血 10% 擋下死劫！`); }
      ...
      const nextMonster = (newHp === 0 || (isDeadFromCounter && monster.isBoss)) ? generateMonsterState(nextRealm, nextQi, nextQiToNext, nextHasAscended) : { ...monster, hp: newHp };
      setMonster(nextMonster);
      setPlayer(p => ({ ...p, realmIndex: nextRealm, qi: nextQi, qiToNext: nextQiToNext, coins: nextCoins, vitality: nextVitality, streakCount: nextStreak, streakShields: nextShields, totalFocusTime: nextTotalFocusTime, artifacts: (isKilled && Math.random()<0.05) ? [...p.artifacts, ARTIFACT_POOL[0].id] : p.artifacts, epiphanyPills: nextPills, hasAscended: nextHasAscended, logs: [...actionLogs.reverse().map(m => `[${new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}] ${m}`), ...(p.logs || [])].slice(0, 100) }));
    ...
    <div className={`min-h-screen text-slate-300 font-mono p-4 flex flex-col items-center overflow-x-hidden relative transition-colors duration-500
      ...
      {toast && (
        <div className={`fixed top-24 left-1/2 -translate-x-1/2 z-[700] flex flex-col items-center p-4 sm:p-6 rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.8)] border backdrop-blur-xl animate-fade-in-down transition-all max-w-[90%] w-full cursor-pointer
            ${toast.type === 'kill' ? 'bg-amber-950/80 border-amber-500/50 text-amber-200' : toast.type === 'focus' ? 'bg-rose-950/80 border-rose-500/50 text-rose-200' : 'bg-cyan-950/80 border-cyan-500/50 text-cyan-200'}`} onClick={() => setToast(null)}>
            <div className="flex items-center gap-3 font-black tracking-widest text-sm sm:text-base mb-2 text-center">
                {toast.type === 'kill' ? <Sword size={20} className="text-amber-400 animate-bounce"/> : toast.type === 'focus' ? <Flame size={20} className="text-rose-400 animate-pulse"/> : <Wind size={20} className="text-cyan-400 animate-pulse"/>} {toast.text}
            </div>
      ...
      {showGiveUpWarning && (
        <div className="fixed inset-0 z-[800] bg-black/95 backdrop-blur-2xl p-6 flex flex-col items-center justify-center font-bold">
          <div className="w-full max-w-lg bg-rose-950/80 p-8 rounded-2xl border border-rose-500/50 shadow-[0_0_80px_rgba(244,63,94,0.3)] flex flex-col items-center text-center">
            ...
            <p className="text-rose-200 text-sm md:text-base leading-relaxed mb-8">突破死劫中，強行收功將遭受極重反噬。確認放棄？</p>
        ...
        <div className="fixed inset-0 z-[700] bg-black/95 backdrop-blur-2xl p-4 flex flex-col items-center justify-center font-bold mt-8">
          <div className="w-full max-w-4xl bg-slate-900/90 p-6 md:p-8 rounded-2xl border border-purple-500/30 flex flex-col max-h-[85vh] shadow-[0_0_50px_rgba(168,85,247,0.2)] relative">
            ...
            <div className="flex flex-col items-center mb-8 border-b border-white/10 pb-6">
                ...
                <h2 className="text-xl md:text-2xl font-black text-purple-400 tracking-widest uppercase">輪迴靈根</h2>
      ...
      {!isActive && (
        <div className="w-full max-w-4xl mb-6 z-10 font-bold px-2 animate-soft-fade-in">
          <div className="flex flex-col items-center mb-8 h-10 justify-center"> <h1 className="text-lg md:text-xl tracking-[1.2em] text-white/30 uppercase font-black drop-shadow-md"> {mode === 'focus' ? '凡人修仙專注' : '靈氣反哺 (調息中)'} </h1> <div className="h-px w-48 bg-gradient-to-r from-transparent via-white/20 to-transparent mt-4 opacity-50"></div> </div>
          <div className={`bg-slate-900/50 backdrop-blur-3xl p-5 md:p-8 rounded-xl border ${mode === 'break' ? 'border-cyan-500/30' : activeColorClass.border} shadow-2xl`}>
              ...
              <div className="flex items-center gap-4 w-full md:flex-1">
                 ...
                 <div className="flex flex-col justify-center min-w-0">
                    <h2 className="text-xl sm:text-2xl font-black tracking-widest uppercase text-white flex items-center flex-wrap">
                      {player.equippedTitle && <span className="text-amber-400 mr-2 border border-amber-500/50 bg-amber-950/50 px-2 py-0.5 rounded text-[10px] tracking-widest">[{TITLE_DATA.find(t=>t.id===player.equippedTitle)?.name}]</span>} {currentRealmData.name}
                    ...
                    <p className={`text-xs md:text-sm mt-2 italic drop-shadow-md ${mode === 'break' ? 'text-cyan-300' : activeColorClass.text}`}>{currentRealmData.desc}</p>
                 </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
              <div className="space-y-3"> <div className="flex justify-between text-[10px] uppercase font-black opacity-60 tracking-widest text-white"> <span>氣血真元</span> <span>{formatNumber(player.vitality)} / {formatNumber(maxVitality)}</span> </div> <div className="h-2 bg-black/60 rounded-full overflow-hidden shadow-inner"> <div className="h-full bg-rose-500 transition-all duration-1000" style={{ width: `${Math.min(100, (player.vitality/maxVitality)*100)}%` }}></div> </div> </div>
              <div className="space-y-3"> <div className="flex justify-between text-[10px] uppercase font-black opacity-60 tracking-widest text-white"> <span>修為進度 {monster.isBoss && !player.hasAscended && <span className="text-rose-400 ml-2 font-black">(突破死劫中)</span>}</span> <span>{formatNumber(player.qi)} / {formatNumber(player.qiToNext)}</span> </div> <div className="h-2 bg-black/60 rounded-full overflow-hidden shadow-inner"> <div className={`h-full ${mode === 'break' ? 'bg-cyan-500' : activeColorClass.bg} transition-all duration-1000`} style={{ width: `${Math.min(100, (player.qi/player.qiToNext)*100)}%` }}></div> </div> </div>
            </div>
          </div>
      ...
      <div className={`w-full max-w-4xl bg-slate-900/40 backdrop-blur-3xl p-8 md:p-14 rounded-2xl border ${mode === 'break' ? 'border-cyan-500/30' : 'border-white/10'} text-center z-10 shadow-2xl transition-all duration-700 flex flex-col justify-center 
        ...
        <div className="flex flex-col items-center justify-center gap-6">
          <div className="flex justify-center gap-6 md:gap-8 font-bold w-full">
            {!isActive ? ( <button onClick={toggleTimer} className={`flex items-center justify-center gap-4 px-16 py-7 hover:text-black border border-white/20 rounded-2xl text-base sm:text-xl font-black tracking-widest uppercase transition-all shadow-2xl backdrop-blur-md w-full md:w-auto ${mode === 'break' ? 'bg-cyan-900/40 text-cyan-300 hover:bg-cyan-400' : 'bg-white/10 hover:bg-white text-white'}`}> <Sparkles size={24}/> {mode === 'focus' ? '運轉周天 (蓄力)' : '開始調息 (回血)'} </button> ) : (
              <button onClick={preCheckGiveUp} className={`flex items-center justify-center gap-4 px-16 py-6 bg-rose-950/30 text-rose-500/50 hover:text-rose-300 hover:bg-rose-900 border border-rose-500/20 rounded-2xl text-sm sm:text-base font-black tracking-widest uppercase transition-all w-full md:w-auto`}> <AlertTriangle size={20}/> 強行收功 </button>
            )}
          </div>
          {!isActive && canUsePill && mode === 'focus' && ( <button onClick={() => handleComplete(true)} className="flex items-center justify-center gap-2 px-8 py-4 rounded-full text-xs font-black tracking-widest bg-amber-900/50 text-amber-400 border border-amber-500/50 shadow-lg animate-soft-fade-in"> <Pill size={16}/> 吞服頓悟丹，瞬間出關 (餘 {player.epiphanyPills}) </button> )}
        </div>
      </div>
      ...
      {!isActive && (
        <div className="w-full max-w-4xl mt-4 z-10 font-bold animate-soft-fade-in">
          <div className="bg-slate-950/90 backdrop-blur-3xl rounded-2xl border border-white/10 shadow-2xl flex flex-col h-[750px] overflow-hidden">
            <div className="flex bg-black/80 border-b border-white/10 p-2 gap-2 overflow-x-auto no-scrollbar flex-shrink-0"> {[ { id: 'log', label: '日誌', icon: History }, { id: 'skills', label: '功法', icon: ScrollText }, { id: 'forge', label: '淬煉', icon: Hammer }, { id: 'artifacts', label: '法寶', icon: Box }, { id: 'companions', label: '紅顏', icon: Heart } ].map(tab => ( <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 py-4 rounded-xl text-xs font-black uppercase flex flex-col items-center gap-2 transition-all min-w-[75px] ${activeTab===tab.id ? 'bg-white/15 text-white border border-white/20' : 'text-white/40 hover:text-white/80'}`}> <tab.icon size={18}/> <span>{tab.label}</span> </button> ))} </div>
            <div className="p-5 md:p-10 overflow-y-auto flex-1 custom-scrollbar">
              {activeTab === 'log' && ( <div className="space-y-4 animate-pop-in pb-10"> {(player.logs || []).map((e, i) => (<div key={i} className={`p-5 rounded-xl border border-white/20 text-[11px] leading-relaxed transition-all ${i===0?'bg-white/10 text-white shadow-lg':'bg-black/60 border-white/10 text-white/40'}`}>{e}</div>))} </div> )}
