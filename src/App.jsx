...
const MAJOR_REALMS_DATA = [
  ...
  { name: '築基境', desc: '靈氣化液，壽元大增，辟除凡胎。', color: 'teal' },
  { name: '結丹境', desc: '丹田結丹，靈力固化。', color: 'blue' },
  { name: '元嬰境', desc: '碎丹成嬰，神魂不滅，瞬移離體。', color: 'indigo' },
  { name: '化神境', desc: '初步掌控天地元氣，具備飛升資格。', color: 'purple' },
  { name: '煉虛境', desc: '化實為虛，掌控空間法則。', color: 'violet' },
...
export default function App() {
  ...
  const [player, setPlayer] = useState(() => {
    try {
      const saved = localStorage.getItem('xianxia_master_v66_final');
  ...
  useEffect(() => { 
    localStorage.setItem('xianxia_master_v66_final', JSON.stringify(player)); 
  ...
  const handleHeal = () => {
    if (player.coins >= healCost && player.vitality < maxVitality) {
      ...
      addLog(`[煉丹] 吞服丹藥，氣血恢復 50%。`);
    }
  };

  const handleGiveUp = () => {
    setIsActive(false); setTargetEndTime(null);
    if (Math.random() < evadeRate) { addLog(`💨 【羅煙閃避】成功閃過反噬！`); }
    else {
      ...
      if (nextHp <= 0) {
        ...
        else { nextHp = Math.floor(maxVitality * 0.5); nextQi = Math.floor(nextQi * 0.8); addLog(`💀 【身死道消】反噬過重，損失 20% 當前修為！`); }
  ...
  return (
    <div className={`min-h-screen text-slate-300 font-mono p-4 flex flex-col items-center overflow-x-hidden relative transition-colors duration-300 
      ...
      <style>{`.custom-scrollbar::-webkit-scrollbar { width: 3px; height: 3px; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 10px; }`}</style>

      <div className={`fixed top-4 right-4 z-50 flex items-center gap-2 bg-emerald-900/80 text-emerald-300 px-3 py-1.5 rounded-full text-[10px] font-bold border border-emerald-500/30 transition-opacity duration-500 ${saveIndicator ? 'opacity-100' : 'opacity-0'}`}><Save size={12}/> 已同步</div>
      ...
      <div className={`w-full max-w-4xl mt-4 transition-all duration-500 z-10 font-bold ${isActive ? 'opacity-0 pointer-events-none translate-y-10' : 'opacity-100 translate-y-0'}`}>
            ...
            {activeTab === 'forge' && (
              <div className="space-y-12 animate-pop-in pb-10">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="p-8 rounded-xl bg-emerald-950/30 border border-emerald-500/30 h-64 flex flex-col justify-between shadow-lg font-bold"><div><h3 className="text-emerald-400 font-black text-xl flex items-center gap-2"><Pill size={20}/> 回春丹</h3><p className="text-white/60 text-[10px] mt-2 italic">瞬間恢復 50% 氣血。</p></div><button onClick={handleHeal} disabled={player.coins < healCost || player.vitality >= maxVitality} className="w-full py-5 bg-emerald-900/60 hover:bg-emerald-600 text-emerald-100 rounded font-black text-xs transition-all disabled:opacity-30 border border-emerald-500/50">煉丹 ({healCost} 靈石)</button></div>
                  <div className="p-8 rounded-xl bg-white/5 border border-white/10 h-64 flex flex-col justify-between font-bold"><div><h3 className="text-white font-black text-xl">祭煉戰力</h3><p className="text-white/50 text-[10px] mt-2 italic">+100 基礎攻擊。</p></div><button onClick={() => { const cost = Math.floor(1000 * Math.pow(1.18, (player.baseCombat - 150) / 100) * forgeDiscount); if(player.coins >= cost) setPlayer(p => ({ ...p, coins: p.coins - cost, baseCombat: p.baseCombat + 100 })) }} className="w-full py-5 bg-white/10 hover:bg-white text-white hover:text-black rounded font-black text-xs shadow-xl transition-all disabled:opacity-30">祭煉 ({Math.floor(1000 * Math.pow(1.18, (player.baseCombat - 150) / 100) * forgeDiscount)} 靈石)</button></div>
                  <div className="p-8 rounded-xl bg-white/5 border border-white/10 h-64 flex flex-col justify-between font-bold"><div><h3 className="text-white font-black text-xl">熬煉體魄</h3><p className="text-white/50 text-[10px] mt-2 italic">+100 基礎氣血。</p></div><button onClick={() => { const cost = Math.floor(1000 * Math.pow(1.18, (player.baseMaxVitality - 100) / 100) * forgeDiscount); if(player.coins >= cost) setPlayer(p => ({ ...p, coins: p.coins - cost, baseMaxVitality: p.baseMaxVitality + 100, vitality: p.vitality + 100 })) }} className="w-full py-5 bg-white/10 hover:bg-white text-white hover:text-black rounded font-black text-xs shadow-xl transition-all disabled:opacity-30">熬煉 ({Math.floor(1000 * Math.pow(1.18, (player.baseMaxVitality - 100) / 100) * forgeDiscount)} 靈石)</button></div>
                </div>
                <div className="space-y-6">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white/5 p-6 rounded-xl border border-white/10 h-48 flex flex-col justify-between shadow-inner font-bold"><div className="flex justify-between text-sm text-white">聚靈大陣 <span className="opacity-50 font-mono">Lv.{player.arrays?.qi||0}</span></div><p className="text-[10px] opacity-60 italic text-white">修為獲取 +5%/級</p><button onClick={() => { if(player.coins >= arrayQiCost) setPlayer(p => ({ ...p, coins: p.coins - arrayQiCost, arrays: {...p.arrays, qi: (p.arrays?.qi||0)+1} })) }} disabled={player.coins < arrayQiCost} className="w-full py-3 bg-white/10 hover:bg-white text-white rounded text-[9px] font-black border border-white/10 transition-all">升級 ({arrayQiCost} 靈石)</button></div>
                      <div className="bg-white/5 p-6 rounded-xl border border-white/10 h-48 flex flex-col justify-between shadow-inner font-bold"><div className="flex justify-between text-sm text-white">顛倒五行陣 <span className="opacity-50 font-mono">Lv.{player.arrays?.def||0}</span></div><p className="text-[10px] opacity-60 italic text-white">反噬減傷 +5%/級</p><button onClick={() => { if(player.coins >= arrayDefCost) setPlayer(p => ({ ...p, coins: p.coins - arrayDefCost, arrays: {...p.arrays, def: (p.arrays?.def||0)+1} })) }} disabled={player.coins < arrayDefCost} className="w-full py-3 bg-white/10 hover:bg-white text-white rounded text-[9px] font-black border border-white/10 transition-all">升級 ({arrayDefCost} 靈石)</button></div>
                   </div>
                </div>
        ...
        <footer className="pt-16 pb-24 text-center text-[9px] font-light text-white/40 tracking-[0.8em] uppercase flex flex-col items-center gap-4">
          <p>《凡人修仙傳》原著設定歸作者 忘語 所有 | V66 封神演義版</p>
          <button onClick={()=>{if(window.confirm('重置修行？所有成果將遺失。')) { localStorage.clear(); window.location.reload(); }}} className="opacity-50 hover:opacity-100 transition-opacity border border-white/20 px-4 py-1.5 rounded-full text-[8px] tracking-widest hover:bg-rose-900/50">散功重修</button>
        </footer>
      </div>
    </div>
  );
