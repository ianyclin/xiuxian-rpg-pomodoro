...
export default function App() {
  ...
  const getMultiplier = (type) => {
    ...
    if (type === 'atk' || type === 'streak_cap') {
      ...
      if (swordCount >= 2) mult += 0.25 * swordCount; 
    }
  ...
  const handleHeal = () => {
    if (player.coins >= healCost && player.vitality < maxVitality) {
      ...
      addLog(`[煉丹] 吞服回春丹，恢復 ${healAmount.toLocaleString()} 點氣血。`);
    }
  };

  const preCheckGiveUp = () => {
    const isBottleneck = monster.name.includes('瓶頸') || monster.name.includes('劫');
    if (isBottleneck) setShowGiveUpWarning(true); else executeGiveUp();
  };

  const executeGiveUp = () => {
    setShowGiveUpWarning(false); setIsActive(false); setTargetEndTime(null);
    if (Math.random() < evadeRate) { addLog(`💨 【羅煙閃避】成功閃避反噬！連擊不墜！`); }
    else {
      ...
      if (nextHp <= 0) {
        ...
        else { nextHp = Math.floor(maxVitality * 0.5); setPlayer(p => ({ ...p, qi: Math.floor(p.qi * 0.8) })); addLog(`💀 【身死道消】反噬過重，損失 20% 修為與連擊！`); }
      } else { addLog(`🚨 【靈力反噬】神魂震盪，承受 ${penalty.toLocaleString()} 傷害。`); }
      let nextStreak = 0; let nextShields = player.streakShields;
      if (player.streakCount > 0 && nextHp > 0) {
        if (player.streakShields > 0) { nextStreak = player.streakCount; nextShields = player.streakShields - 1; addLog(`🛡️ 【法寶護主】消耗 1 層連擊護盾抵擋反噬，連擊未中斷！`); }
        else { addLog(`📉 靈壓潰散，連擊歸零。`); }
      }
  ...
  const handleComplete = () => {
    ...
    if (mode === 'focus') {
      ...
      let nextVitality = player.vitality; let nextRealm = player.realmIndex;
      let nextQiToNext = player.qiToNext; let nextHistory = player.history;
      let newArtifacts = [...(player.artifacts || [])]; let nextHasAscended = player.hasAscended;
      if (isCrit && Math.random() < 0.20) {
        const lifesteal = Math.floor(maxVitality * (getMultiplier('lifesteal') - 1));
        if (lifesteal > 0) { nextVitality = Math.min(maxVitality, nextVitality + lifesteal); addLog(`💉 【真靈吸吮】爆擊恢復了 ${lifesteal.toLocaleString()} 氣血！`); }
      }
      let killLog = '';
      if (newHp === 0) {
        ...
        if (isFinalBoss && !player.hasAscended) {
            ...
            setCelebration({ name: REALMS[nextRealm].name }); killLog = `☄️ 【突破】晉升至${REALMS[nextRealm].name}！` + killLog;
        } else { killLog = `⚔️ 【擊殺】奪得修為 ${killQi.toLocaleString()}！` + killLog; }
        setMonster(generateMonsterState(nextRealm));
      } else { setMonster(prev => ({ ...prev, hp: newHp })); }
      const dmgLog = isCrit ? `🔥 【爆擊】造成 ${actualDamage.toLocaleString()} 傷害。` : `[運功] 造成 ${actualDamage.toLocaleString()} 傷害。`;
      addLog(`${dmgLog} ${killLog || `獲修為 ${passiveQi.toLocaleString()}。`}`);
      setPlayer(p => ({
          ...p, realmIndex: nextRealm, qi: nextQi, qiToNext: nextQiToNext, coins: nextCoins,
          vitality: nextVitality, streakCount: p.streakCount + 1, streakShields: maxStreakShields,
          totalFocusTime: (p.totalFocusTime || 0) + focusDuration, artifacts: newArtifacts,
          history: nextHistory, hasAscended: nextHasAscended
      }));
      setMode('break'); setTimeLeft(5 * 60);
    } else { 
      setMode('focus'); setTimeLeft(focusDuration); 
      const heal = Math.floor(maxVitality * healPct);
      setPlayer(p => ({ ...p, vitality: Math.min(maxVitality, p.vitality + heal), streakShields: maxStreakShields }));
      addLog(`🌧️ 【靈雨降臨】運轉周天圓滿，恢復了 ${heal.toLocaleString()} 氣血。`); 
    }
  };
  ...
  const handleGacha = () => {
    ...
    const currentLuck = getMultiplier('luck_floor');
    const roll = Math.random(); let targetRarity = 'COMMON';
  ...
  const InsightsChart = () => {
    ...
    if (data.length < 2) return <div className="h-full flex items-center justify-center text-white/10 tracking-widest font-bold text-xs uppercase">識海未成，尚無投影</div>;
    const maxT = Math.max(...data.map(d => d.time || 0)) || 1;
    const points = data.map((d, i) => { const x = (i / (data.length - 1)) * 100; const y = 100 - ((d.time || 0) / maxT) * 100; return `${x},${y}`; }).join(' ');
    return (
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full">
        <polyline fill="none" stroke="rgba(16, 185, 129, 0.4)" strokeWidth="1.5" points={points} />
        {data.map((d, i) => { const cx = (i / (data.length - 1)) * 100; const cy = 100 - ((d.time || 0) / maxT) * 100; return <circle key={i} cx={cx} cy={cy} r="1.5" fill="#fff" className="animate-pulse" />; })}
      </svg>
    );
  };

  return (
    <div className={`min-h-screen text-slate-300 font-mono p-4 flex flex-col items-center overflow-x-hidden relative transition-colors duration-300 pt-10 ${isCollapsing ? 'bg-red-950/80 animate-shake' : isKilling ? 'bg-emerald-950/60' : isCritStrike ? 'bg-rose-900/40' : 'bg-[#020617]'}`} style={{ backgroundImage: 'url("https://images.unsplash.com/photo-1542224566-6e85f2e6772f?auto=format&fit=crop&q=80")', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      <div className="fixed top-0 left-0 w-full bg-emerald-950/90 text-[10px] sm:text-xs py-2 text-center font-black tracking-widest z-[600] border-b border-emerald-500/30 flex flex-wrap items-center justify-center gap-x-6 gap-y-1 shadow-[0_0_15px_rgba(16,185,129,0.3)]">
        <div className="flex items-center gap-1.5 text-emerald-400"><Network size={14} className="animate-pulse" /><span>三千世界運轉:</span><span className="text-white font-mono text-sm">{globalStats.focus.toLocaleString()}</span></div>
        <div className="hidden sm:block text-emerald-700">|</div>
        <div className="flex items-center gap-1.5 text-yellow-400"><Crown size={14} className="animate-pulse drop-shadow-[0_0_5px_rgba(250,204,21,0.8)]" /><span>飛升仙人:</span><span className="text-white font-mono text-sm">{globalStats.ascensions.toLocaleString()}</span></div>
      </div>
      ...
      {showGiveUpWarning && (
        <div className="fixed inset-0 z-[600] bg-black/95 backdrop-blur-2xl p-6 flex flex-col items-center justify-center font-bold">
          <div className="w-full max-w-lg bg-rose-950/80 p-8 rounded-2xl border border-rose-500/50 shadow-[0_0_80px_rgba(244,63,94,0.3)] flex flex-col items-center text-center">
            ...
            <p className="text-rose-200 text-sm md:text-base leading-relaxed mb-8">當前正處於 <span className="text-white font-black underline decoration-rose-500 decoration-2">大瓶頸/雷劫</span>！<br/>此時強行出關將遭受極為嚴重的反噬。<br/><br/>確認要放棄本次突破嗎？</p>
            <div className="flex gap-4 w-full"><button onClick={() => setShowGiveUpWarning(false)} className="flex-1 py-4 bg-white/10 hover:bg-white text-white hover:text-black rounded-xl text-sm font-black transition-all border border-white/20 uppercase tracking-widest">繼續運功</button><button onClick={executeGiveUp} className="flex-1 py-4 bg-rose-600 hover:bg-rose-500 text-white rounded-xl text-sm font-black transition-all shadow-lg uppercase tracking-widest">強行中斷</button></div>
          </div>
        </div>
      )}
      <div className={`fixed top-14 right-4 z-50 flex items-center gap-2 bg-emerald-900/80 text-emerald-300 px-4 py-2 rounded-full text-xs font-bold border border-emerald-500/30 transition-opacity duration-500 ${saveIndicator ? 'opacity-100' : 'opacity-0'}`}><Save size={14} className="animate-pulse"/> 天道已同步</div>
      {showRealmGuide && (
        <div className="fixed inset-0 z-[400] bg-black/95 backdrop-blur-xl p-4 md:p-8 flex flex-col items-center justify-center font-bold mt-8">
          <div className="w-full max-w-4xl flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4 flex-shrink-0"><h2 className="text-xl md:text-2xl font-black text-white tracking-widest uppercase flex items-center gap-3"><BookOpen className="text-emerald-500"/> 天道經緯 (境界全覽)</h2><button onClick={() => setShowRealmGuide(false)} className="p-4 hover:bg-white/10 rounded-full transition-all text-white/50 hover:text-white"><X size={24}/></button></div>
            <div className="w-full overflow-y-auto custom-scrollbar bg-black/40 rounded-xl border border-white/5 flex-1"><table className="w-full text-left border-collapse min-w-[600px]"><thead><tr className="text-xs text-white/30 uppercase tracking-widest border-b border-white/5"><th className="py-4 px-4">位階範圍</th><th className="py-4 px-4">境界名號</th><th className="py-4 px-4">神識導讀</th></tr></thead><tbody>{GUIDE_REALMS.map((r, i) => (<tr key={i} className={`border-b border-white/5 transition-colors hover:bg-white/5`}><td className="py-4 px-4 font-mono text-xs text-white/30">{r.range}</td><td className={`py-4 px-4 font-black text-sm text-white/80`}>{r.name}</td><td className="py-4 px-4 text-xs text-white/50 leading-relaxed italic">{r.desc}</td></tr>))}</tbody></table></div>
          </div>
        </div>
      )}
      {showGuide && (
        <div className="fixed inset-0 z-[400] bg-black/95 backdrop-blur-xl p-4 flex flex-col items-center justify-center font-bold mt-8">
          <div className="w-full max-w-2xl bg-slate-900/50 p-6 md:p-8 rounded-2xl border border-white/10 shadow-2xl flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4 flex-shrink-0"><h2 className="text-lg md:text-xl font-black text-white tracking-widest uppercase flex items-center gap-3"><HelpCircle className="text-emerald-400"/> 修行指引與祕訣</h2><button onClick={() => setShowGuide(false)} className="p-4 hover:bg-white/10 rounded-full transition-all text-white/50 hover:text-white"><X size={24}/></button></div>
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-6 pb-6"><div className="flex gap-2 bg-black/40 p-1 rounded-lg border border-white/5 flex-shrink-0"><button onClick={() => setGuideTab('rules')} className={`flex-1 py-4 text-xs md:text-sm font-bold rounded uppercase tracking-widest transition-all ${guideTab === 'rules' ? 'bg-white/10 text-white shadow-inner' : 'text-white/40 hover:text-white/80'}`}>基礎法則</button><button onClick={() => setGuideTab('tips')} className={`flex-1 py-4 text-xs md:text-sm font-bold rounded uppercase tracking-widest transition-all ${guideTab === 'tips' ? 'bg-amber-500/20 text-amber-400 shadow-inner' : 'text-amber-400/40 hover:text-amber-400/80'}`}>修行祕訣 (Tips)</button></div>
              {guideTab === 'rules' ? (
                <div className="space-y-4 text-sm leading-relaxed animate-pop-in">
                   <section className="bg-white/5 p-5 rounded-lg border border-white/5"><h3 className="text-emerald-400 text-base mb-2 flex items-center gap-2 font-black"><Play size={18}/> 運轉周天 (專注計時)</h3><p className="text-white/70">完成計時唯有擊殺妖獸才能獲得爆量資源。此版本支援「離線修行」。</p></section>
                   <section className="bg-white/5 p-5 rounded-lg border border-white/5"><h3 className="text-rose-400 text-base mb-2 flex items-center gap-2 font-black"><Square size={18}/> 走火入魔 (強行出關)</h3><p className="text-white/70">計時中途放棄會扣除百分比氣血與連擊。後期反噬兇猛，具備 80% 氣血保護鎖。</p></section>
                   <section className="bg-white/5 p-5 rounded-lg border border-white/5"><h3 className="text-yellow-400 text-base mb-2 flex items-center gap-2 font-black"><Pill size={18}/> 靈丹妙藥 (主動恢復)</h3><p className="text-white/70">在「洞府淬煉」可煉製回春丹。消耗靈石即可瞬間拉回血線。</p></section>
                </div>
              ) : (
                <div className="space-y-4 text-sm leading-relaxed animate-pop-in">
                   <section className="bg-white/5 p-5 rounded-lg border-l-4 border-emerald-500"><h3 className="text-emerald-400 text-base mb-2 flex items-center gap-2 font-black"><Lightbulb size={18}/> 法寶共鳴 (V70)</h3><p className="text-white/70">特定類型的法寶（如劍類）同時裝備時，會觸發隱藏的「共鳴」加成，大幅提升連擊上限與戰力。</p></section>
                   <section className="bg-white/5 p-5 rounded-lg border-l-4 border-rose-500"><h3 className="text-rose-400 text-base mb-2 flex items-center gap-2 font-black"><Lightbulb size={18}/> 瀕死生機</h3><p className="text-white/70">天道慈悲。強行出關受傷極限為當前氣血 80%。若不幸戰死，損失 20% 修為。請善用《大衍決》減少此傷害。</p></section>
                </div>
              )}
            </div>
          </div>
      ...
      {showStatsReport && (
        <div className="fixed inset-0 z-[400] bg-black/95 backdrop-blur-xl p-4 flex flex-col items-center justify-center font-bold mt-8">
          <div className="w-full max-w-2xl bg-slate-900/50 p-6 md:p-8 rounded-2xl border border-cyan-900/50 shadow-2xl flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4 flex-shrink-0"><h2 className="text-lg md:text-xl font-black text-cyan-400 tracking-widest uppercase flex items-center gap-3"><BarChart3 size={24}/> 屬性極限與轉化報告</h2><button onClick={() => setShowStatsReport(false)} className="p-4 hover:bg-white/10 rounded-full text-white/50"><X size={24}/></button></div>
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4"><h3 className="text-xs text-white/40 uppercase border-b border-white/5 pb-2">基礎倍率 (Base Multipliers)</h3>
                ...
                <div className="space-y-4"><h3 className="text-xs text-white/40 uppercase border-b border-white/5 pb-2">天道法則 (V70 Logic)</h3>
                  ...
                  <div className="flex justify-between text-sm"><span className="text-slate-400 flex flex-col">爆擊率 <span className="text-xs opacity-50 font-normal mt-1">(溢出轉化爆傷 1:3)</span></span><span className="text-purple-400 font-mono mt-1">{(critRate * 100).toFixed(1)}%</span></div>
                  <div className="flex justify-between text-sm"><span className="text-slate-400 flex flex-col">閃避免傷率 <span className="text-xs opacity-50 font-normal mt-1">(溢出轉化連擊 1:0.5)</span></span><span className="text-emerald-400 font-mono">{(evadeRate * 100).toFixed(1)}%</span></div>
      ...
      {celebration && (
        <div className="fixed inset-0 z-[300] bg-black/95 backdrop-blur-2xl flex flex-col items-center justify-center p-12 cursor-pointer font-bold mt-8" onClick={() => setCelebration(null)}>
          <Crown size={80} className="text-yellow-500/80 mb-6 animate-bounce" /><h2 className="text-3xl font-black text-white mb-2 uppercase tracking-widest">{celebration.name.includes('成就真仙') ? '渡劫成功' : '突破瓶頸'}</h2><p className="text-xl text-emerald-400 font-light tracking-widest text-center">【{celebration.name}】</p>
        </div>
      )}
      <div className="w-full max-w-4xl mb-6 transition-focus z-10 font-bold px-2 md:px-0 mt-10">
        <div className="flex flex-col items-center mb-8 h-10 justify-center"><h1 className="text-lg md:text-xl font-extralight tracking-[1.2em] text-white/30 uppercase font-bold drop-shadow-md">凡人修仙專注</h1><div className="h-px w-48 bg-gradient-to-r from-transparent via-white/20 to-transparent mt-4 opacity-50"></div></div>
        <div className={`bg-slate-900/50 backdrop-blur-3xl p-5 md:p-8 rounded-xl border ${themeBorderClass} relative shadow-2xl shadow-inner group transition-all duration-500`}>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/10 pb-6 mb-6">
            <div className="flex items-center gap-4 w-full md:flex-1 min-w-0"><Shield size={36} className={`${themeColorClass} flex-shrink-0`}/><div className="flex flex-col justify-center flex-1 min-w-0"><h2 className="text-2xl font-black tracking-widest uppercase text-white font-bold drop-shadow-lg truncate">{currentRealmData.name}</h2><p className={`text-xs md:text-sm leading-tight ${themeColorClass} font-bold mt-2 opacity-90 italic drop-shadow-md truncate`}>{currentRealmData.desc}</p></div></div>
            <div className="grid grid-cols-2 sm:flex sm:flex-row sm:flex-nowrap justify-start md:justify-end items-start md:items-end gap-x-4 gap-y-6 w-full md:w-auto mt-2 md:mt-0">
               <div className="flex flex-col items-start md:items-end"><span className="text-xs text-yellow-500 uppercase font-black flex items-center gap-1.5 mb-1"><Coins size={12}/> 靈石</span><span className="text-base text-yellow-500 font-mono font-bold drop-shadow-md">{Math.floor(player.coins).toLocaleString()}</span></div>
               <div className="flex flex-col items-start md:items-end"><span className="text-xs text-cyan-400 uppercase font-black flex items-center gap-1.5 mb-1"><Zap size={12}/> SP</span><span className="text-base text-cyan-400 font-mono font-bold drop-shadow-md">{availableSP.toLocaleString()}</span></div>
               <div className="flex flex-col items-start md:items-end"><span className="text-xs text-rose-500 uppercase font-black flex items-center gap-1.5 mb-1"><Sword size={12}/> 連擊</span><span className={`text-base text-rose-500 font-mono font-bold drop-shadow-md transition-all duration-500 flex items-center gap-1 ${comboMultiplier > 2.0 ? 'text-rose-300 scale-110 animate-pulse drop-shadow-[0_0_10px_rgba(244,63,94,0.8)]' : ''}`}>x{comboMultiplier.toFixed(2)} {maxStreakShields > 0 && <span className="text-cyan-400 text-xs ml-1 flex items-center">🛡️{player.streakShields}</span>}</span></div>
               <div className="flex flex-col items-start md:items-end font-bold"><span className="text-xs text-emerald-400 uppercase font-black flex items-center gap-1.5 mb-1"><Clover size={12}/> 氣運</span><span className={`text-base text-emerald-400 font-mono font-bold drop-shadow-md transition-all duration-500 ${getMultiplier('luck_floor') > 1.5 ? 'text-yellow-400 scale-110 animate-bounce drop-shadow-[0_0_10px_rgba(250,204,21,0.8)]' : ''}`}>x{getMultiplier('luck_floor').toFixed(2)}</span></div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
            <div className="space-y-3 relative z-10">
                <div className="flex justify-between text-xs uppercase font-black opacity-60 tracking-widest text-white"><span className="flex items-center gap-2">氣血真元 {player.vitality < maxVitality && <span className="text-emerald-400 animate-pulse font-normal">(靈雨降臨)</span>}</span><span>{Math.floor(player.vitality).toLocaleString()} / {maxVitality.toLocaleString()}</span></div>
                <div className={`h-2.5 bg-black/60 rounded-full overflow-hidden shadow-inner transition-all duration-300 ${isHealing ? 'shadow-[0_0_15px_rgba(16,185,129,0.8)]' : ''}`}><div className="h-full bg-rose-500 transition-all duration-1000 shadow-[0_0_10px_#f43f5e]" style={{ width: `${(player.vitality/maxVitality)*100}%` }}></div></div>
            </div>
            <div className="space-y-3 relative z-10">
              <div className="flex justify-between text-xs uppercase font-black opacity-60 tracking-widest text-white"><span>修為進度</span><span>{Math.floor(player.qi).toLocaleString()} / {player.qiToNext.toLocaleString()}</span></div>
              <div className="h-2.5 bg-black/60 rounded-full overflow-hidden shadow-inner"><div className={`h-full bg-${currentRealmData.color}-500 transition-all duration-1000 shadow-[0_0_10px_currentColor]`} style={{ width: `${(player.qi/player.qiToNext)*100}%` }}></div></div>
            </div>
          </div>
        </div>
        <div className="mt-6 flex flex-col gap-5">
           <div className="flex overflow-x-auto py-5 px-4 gap-8 bg-black/40 rounded-xl border border-white/10 flex-shrink-0 custom-scrollbar">{REALMS.map((r, i) => (<div key={i} className={`flex flex-col items-center min-w-[90px] transition-all relative ${i===player.realmIndex?'scale-110 opacity-100':'opacity-40'}`}><div className={`w-10 h-10 rounded-lg border flex items-center justify-center font-black text-sm rotate-45 transition-all ${i===player.realmIndex?'bg-white text-black rotate-0 shadow-2xl':'border-white/20 text-white'}`}><span>{i}</span></div><span className="text-xs font-black mt-5 whitespace-nowrap uppercase tracking-tighter">{r.name}</span></div>))}</div>
           <div className="flex flex-wrap justify-center md:justify-end gap-4 mt-2">
             <button onClick={() => setShowGuide(true)} className="flex items-center gap-2 text-xs font-black text-emerald-400 hover:text-emerald-300 transition-all bg-white/5 px-5 py-3 rounded-full border border-white/10 backdrop-blur-md uppercase tracking-tighter"><HelpCircle size={16}/> 修行指引</button>
             <button onClick={() => setShowStatsReport(true)} className="flex items-center gap-2 text-xs font-black text-cyan-400 hover:text-cyan-300 transition-all bg-white/5 px-5 py-3 rounded-full border border-white/10 backdrop-blur-md uppercase tracking-tighter"><BarChart3 size={16}/> 屬性極限</button>
             <button onClick={() => setShowRealmGuide(true)} className="flex items-center gap-2 text-xs font-black text-white/60 hover:text-white transition-all bg-white/5 px-5 py-3 rounded-full border border-white/10 backdrop-blur-md uppercase tracking-tighter"><BookOpen size={16}/> 境界全覽</button>
           </div>
        </div>
      </div>
      <div className={`w-full max-w-4xl bg-slate-900/40 backdrop-blur-3xl p-8 md:p-14 rounded-2xl border border-white/10 text-center mb-8 z-10 shadow-2xl transition-all duration-700 ${isActive ? 'scale-[1.02] shadow-[0_0_50px_rgba(16,185,129,0.15)] border-emerald-500/30' : ''}`}>
        ...
        <div className="flex justify-center gap-6 md:gap-8 font-bold">
          {!isActive ? (<button onClick={toggleTimer} className="flex items-center justify-center gap-4 px-10 md:px-16 py-5 md:py-7 bg-white/10 hover:bg-white text-white hover:text-black border border-white/20 rounded-2xl text-lg md:text-xl font-black tracking-[0.5em] uppercase transition-all shadow-2xl backdrop-blur-md w-full md:w-auto"><Sparkles size={24} className="fill-current animate-pulse"/> 運轉周天</button>) : (
            <button onClick={preCheckGiveUp} className="flex items-center justify-center gap-4 px-10 md:px-16 py-5 md:py-7 bg-rose-950/50 text-rose-400 hover:bg-rose-900/80 border border-rose-500/40 rounded-2xl text-lg md:text-xl font-black uppercase active:scale-95 transition-all shadow-2xl backdrop-blur-md w-full md:w-auto"><AlertTriangle size={24} className="fill-current"/> 強行出關</button>
          )}
        </div>
      </div>
      <div className={`w-full max-w-4xl mt-4 transition-all duration-500 z-10 font-bold ${isActive ? 'opacity-0 pointer-events-none translate-y-10' : 'opacity-100 translate-y-0'}`}>
        <div className="bg-slate-950/90 backdrop-blur-3xl rounded-2xl border border-white/10 shadow-2xl flex flex-col h-[800px] overflow-hidden">
          <div className="flex bg-black/80 border-b border-white/10 p-2 gap-2 overflow-x-auto no-scrollbar flex-shrink-0">
            {[{ id: 'skills', label: '功法祕籍', icon: ScrollText }, { id: 'forge', label: '洞府淬煉', icon: Hammer }, { id: 'artifacts', label: '法寶庫', icon: Box }, { id: 'insights', label: '識海投影', icon: Activity }, { id: 'log', label: '修行日誌', icon: History }].map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex-1 py-4 md:py-5 rounded-xl text-xs md:text-sm font-black uppercase flex flex-col items-center justify-center gap-2 transition-all min-w-[80px] ${activeTab===tab.id ? 'bg-white/15 text-white shadow-inner border border-white/20' : 'text-white/40 hover:text-white/80 hover:bg-white/5'}`}><tab.icon size={18} className="md:size-[20px]"/> <span>{tab.label}</span></button>
            ))}
          </div>
          <div className="p-5 md:p-10 overflow-y-auto flex-1 custom-scrollbar">
            {activeTab === 'skills' && (
              <div className="space-y-14 animate-pop-in">
                <div><h3 className="text-white/60 text-sm font-black uppercase border-b border-white/20 pb-4 mb-8 tracking-widest flex justify-between"><span>凡俗根基 (SP 研習)</span><span className="text-cyan-400">可用 SP: {availableSP.toLocaleString()}</span></h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">{BASIC_SKILLS.map(s => { const lvl = player.basicSkills?.[s.id] || 0; return (
                    <div key={s.id} className="p-5 rounded-xl border border-white/20 bg-white/5 flex flex-col justify-between h-56 shadow-inner group">
                       ...
                       <button onClick={() => { if(availableSP >= 1 && lvl < s.maxLvl) setPlayer(p => ({...p, basicSkills: {...p.basicSkills, [s.id]: lvl+1}})) }} disabled={availableSP < 1 || lvl >= s.maxLvl} className="mt-5 w-full py-4 bg-white/10 hover:bg-cyan-600 hover:border-cyan-400 text-white rounded-lg text-xs font-black border border-white/20 transition-all disabled:opacity-30">研習 (1 SP)</button>
                    </div>);})}</div>
                </div>
                <div><h3 className="text-white/60 text-sm font-black uppercase border-b border-white/20 pb-4 mb-8 tracking-widest">機緣祕籍 (13 種)</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">{SECRET_BOOKS.map(book => { const lvl = player.secretBooks?.[book.id] || 0; const learned = lvl > 0; const upCost = Math.floor(10000 * Math.pow(2, lvl) * forgeDiscount); return (
            ...
            {activeTab === 'forge' && (
              <div className="space-y-14 animate-pop-in pb-10">
                ...
                <div className="bg-gradient-to-br from-white/10 to-transparent p-8 md:p-14 rounded-2xl border border-white/20 text-center relative overflow-hidden mt-8"><h3 className="text-white font-black text-2xl uppercase mb-8 tracking-widest flex items-center justify-center gap-3"><Compass className="text-yellow-400"/> 萬寶樓尋寶</h3>
                  <div className="flex justify-center gap-6 mb-12 overflow-x-auto pb-6 custom-scrollbar">{Object.entries(RARITY).map(([k, r]) => (<div key={k} className="flex flex-col items-center min-w-[80px] opacity-80"><span className={`text-xs font-black uppercase ${r.color} drop-shadow-md`}>{r.name}</span><span className="text-sm font-mono mt-2 text-white">{(r.weight*100*getMultiplier('luck_floor')).toFixed(1)}%</span></div>))}</div>
                  <button onClick={handleGacha} disabled={player.coins < gachaCost} className="px-8 md:px-20 py-6 md:py-8 bg-white/15 hover:bg-white text-white hover:text-black font-black rounded-2xl shadow-2xl transition-all border border-white/30 disabled:opacity-30 flex items-center justify-center gap-4 mx-auto text-base md:text-lg uppercase tracking-[0.4em]"><Sparkles size={24}/> 尋寶 ({Math.floor(gachaCost).toLocaleString()} 靈石)</button>
                </div>
              </div>
            )}
            {activeTab === 'artifacts' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 animate-pop-in pb-10">{ARTIFACT_POOL.map(art => {
            ...
            {activeTab === 'insights' && (
              <div className="h-[500px] md:h-[600px] animate-pop-in bg-black/60 rounded-2xl border border-white/20 shadow-inner p-6 md:p-12 flex flex-col"><div className="flex justify-between items-center mb-10 opacity-70 text-xs font-black uppercase tracking-[0.4em] text-white"><span className="flex items-center gap-3"><Activity size={16}/> 識海投影 (修煉進程)</span><span>累計時間: {Math.floor((player.totalFocusTime || 0)/60).toLocaleString()}m</span></div><div className="flex-1 relative border-l-2 border-b-2 border-white/20"><InsightsChart /></div></div>
            )}
            {activeTab === 'log' && (
              <div className="space-y-4 md:space-y-6 animate-pop-in pb-10">{(player.logs || []).map((e, i) => (<div key={i} className={`p-5 md:p-6 rounded-xl border border-white/20 text-xs md:text-sm leading-relaxed transition-all ${i===0?'bg-white/20 text-white shadow-xl animate-pulse':'bg-black/60 border-white/10 text-white/60'}`}>{e}</div>))}</div>
            )}
          </div>
        </div>
