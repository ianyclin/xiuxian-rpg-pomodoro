  ...
  const renderCompanionLog = (text) => {
    ...
    return (
      <span>
        {parts.map((part, i) => {
          if (part.match(regex)) {
            return <span key={i} className="text-pink-400 font-black drop-shadow-[0_0_8px_rgba(244,114,182,0.8)] px-0.5">{part}</span>;
          }
...
{activeTab === 'log' && (
              <div className="space-y-4 md:space-y-6 animate-pop-in pb-10">
                {(player.logs || []).map((e, i) => {
                  const companionNames = COMPANIONS.map(c => c.name).join('|');
                  const isCompanionLine = new RegExp(`(【(?:${companionNames})】)`).test(e);
                  
                  return (
                    <div key={i} className={`p-5 md:p-6 rounded-xl border leading-relaxed transition-all whitespace-pre-wrap text-xs md:text-sm ${
                      isCompanionLine 
                        ? 'bg-pink-950/30 border-pink-500/50 text-pink-100/90 shadow-[0_0_15px_rgba(244,114,182,0.15)]' 
                        : i === 0 
